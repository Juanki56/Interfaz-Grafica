/**
 * Reconstruye archivos aplicando StrReplace/Write/ApplyPatch del transcript
 * hasta el primer `git checkout HEAD -- src/components/`.
 * Base: commit c7cf3f7. Salida: ./_recovered_pre_checkout/
 */
import fs from 'fs';
...
for (let i = 0; i < lines.length; i++) {
  ...
  for (const part of parts) {
    ...
    if (name === 'StrReplace' && input.path && input.old_string != null && input.new_string != null) {
      ...
      const content = fileState.get(rel);
