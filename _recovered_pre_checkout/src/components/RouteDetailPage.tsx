...
export function RouteDetailPage({ routeId, onViewChange }: RouteDetailPageProps) {
  ...
  const handleBookingClick = () => {
    ...
    if (
      user?.role === 'client' &&
      !documentoTitularCompletoValidoParaReserva(user.tipo_documento, user.numero_documento)
    ) {
      toast.error(
        'Completa tipo y número de documento válidos en Mi Perfil (no valores marcador como solo “-”) antes de solicitar esta ruta.',
      );
  ...
  const goNextHero = () => {
    setHeroIndex((i) => {
      const g =
        imageGallery.length > 0 ? imageGallery : [route.imagen_url || FALLBACK_ROUTE_IMAGE];
      const n = g.length;
      return i >= n - 1 ? 0 : i + 1;
    });
  };
