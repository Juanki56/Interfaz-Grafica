...
function mapServiciosApiAFincaReserva(rows: Servicio[]): FincaReservaServicioItem[] {
  ...
  for (const s of rows) {
    if (!isServicioCatalogoActivo(s)) continue;
...
/** Mantiene el orden devuelto por el API. */
  ...
  useEffect(() => {
    ...
    const loadFarm = async () => {
      try {
        ...
        const orderedStorage = orderGalleryUrls(uniqueImageUrls(imagenes));
        const thumb = orderedStorage[0];
